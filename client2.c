...
#define C_UDPPORT "33107"    // the UDP port of Client 2
#define REQMSG "Client2 doc2"    // registration message sending to the directory server
#define MAXBUFLEN 100

int contactServer(char *addr[],char tcpport[],char serverNum[]){
    int sockfd, numbytes;  
	char buf[MAXBUFLEN];
	...
	printf("Phase 3:Client 2 has dynamic TCP port number and IP address\n");
	...
	if (send(sockfd, REQMSG, 12, 0) != -1)
		//perror("send");
        printf("Phase 3:The file request from Client 2 has been sent to the File Server %s.\n",serverNum);
		
	if ((numbytes = recv(sockfd, buf, MAXBUFLEN-1, 0)) == -1) {
	    perror("recv");
	    exit(1);
	}
	printf("Phase 3:Client 2 received %s from File Server %s\n",buf,serverNum);
...
int main(int argc, char *argv[])
{
	...
	char buf[MAXBUFLEN],tcpport[5],num[1];
	...
	printf("Phase 2:Client 2 has UDP port number %d and IP address \n", atoi(C_UDPPORT));      // On-screen message upon startup of phase 2
	...
	printf("Phase 2:The File requested by Client 2 is present in File Server %s and the File Server's TCP port number is %s.\n",num,tcpport);
	printf("Phase 2:End of Phase 2 for Client 2.\n");      // On-screen message at the end of phase 2
	contactServer(&argv[1],tcpport,num);
