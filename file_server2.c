...
#define TCPPORT "42107"    // the TCP port users will be connecting to in phase 3
#define UDPPORT "21107"    // the UDP port used to register
#define S_UDPPORT "23107"  // the UDP port of File Server
#define REGMSG "File_Server2 42107"    // registration message sending to the directory server
#define BACKLOG 10	 // how many pending connections queue will hold
#define MAXBUFLEN 100         // used when File Server 2 acts as the server
...
int contactClient(){
	...
	printf("Phase 3:File Server 2 has TCP port %d and IP address 68.181.201.3\n",atoi(TCPPORT));
	...
	while(i < 2) {  // main accept() loop
		...
		printf("Phase 3:File server 2 received the request from the Client %d for the file %s.\n", clientNum,req1);
		
		if (send(new_fd, req1, 4, 0) == -1)
			perror("send");
		printf("Phase 3:File Server 2 has sent %s to client %d.\n",req1,clientNum);
		close(new_fd);
	}
...
int main(int argc, char *argv[])                                      /* need to input the IP addr and port number in the command line!!!!!!!! */
{
	...
	printf("Phase 1:File Server 2 has UDP port nnumber %d and IP address ", atoi(S_UDPPORT));            // On-screen message of Server2. What's the IP address?????????????????????
