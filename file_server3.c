...
#define TCPPORT "43107"    // the TCP port users will be connecting to in phase 3
#define UDPPORT "21107"    // the UDP port used to register
#define S_UDPPORT "24107"  // the UDP port of File Server
#define REGMSG "File_Server3 43107"    // registration message sending to the directory server
#define BACKLOG 10	 // how many pending connections queue will hold
#define MAXBUFLEN 100         // used when File Server 3 acts as the server
...
int contactClient(){
	...
	printf("Phase 3:File Server 3 has TCP port %d and IP address 68.181.201.3\n",atoi(TCPPORT));
	...
	while(i < 2) {  // main accept() loop
		...
		printf("Phase 3:File server 3 received the request from the Client %d for the file %s.\n", clientNum,req1);
		
		if (send(new_fd, req1, 4, 0) == -1)
			perror("send");
		printf("Phase 3:File Server 3 has sent %s to client %d.\n",req1,clientNum);
		close(new_fd);
	}
	close(sockfd); // child doesn't need the listener
}

int main(int argc, char *argv[])                                      /* need to input nunki.usc.edu in the command line!!!!!!!! */
{
    int udp_sockfd;
	...
	printf("Phase 1:File Server 3 has UDP port nnumber %d and IP address ", atoi(S_UDPPORT));            // On-screen message of Server3. What's the IP address?????????????????????
